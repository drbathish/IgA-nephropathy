...
5. **Deduplicate before recording.** Match candidates across tools/queries by DOI or PMID first, then by normalized title + publication year. Never create two source records for the same paper; if a duplicate is found via a new tool, add that tool as an additional verification source on the existing record instead.
   ...
   If you discover a record was filed under a wrong identifier (e.g., a guessed DOI later corrected), do not delete the original file — per the project's audit-trail rule, overwrite it with a stub containing exactly `- Verification status: superseded` and `- Superseded by: sources/papers/<correct-file>.md`, plus a one-paragraph explanation of what was wrong and how it was caught. The explicit `superseded` status keeps it mechanically distinguishable from a real candidate in any QC check or eligibility gate that scans for `Verification status:` fields.
